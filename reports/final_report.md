...
Do đặc thù của bài toán phân tích cảm xúc, các phản hồi tiêu cực mang tính chất quyết định và cần được giám sát chặt chẽ. Do đó, nhóm quyết định tập trung tối ưu hóa độ bao phủ (Recall) trên lớp Tiêu cực (Negative) nhằm hạn chế tối đa việc bỏ sót các đánh giá này. Đồng thời, để tránh hạn chế việc làm phiền những trường hợp Tích cực (Positive) bị dự đoán sai, có thể set thêm ngưỡng threshold rating (VD: > 3 sao sẽ được fix lại thành Tích cực) và sau đó check lại thủ công.

Nhóm đã tiến hành thực hiện kỹ thuật tối ưu hóa ngưỡng quyết định (Threshold Tuning) dựa trên các mức Recall mục tiêu để cân bằng giữa rủi ro bỏ sót và hiệu suất chung của mô hình. Kết quả thực nghiệm cho thấy, tại ngưỡng xác suất phân loại $t = 0.74$, mô hình đạt được mức Recall tối ưu cho lớp Tiêu cực là **96%**, đồng thời giữ các chỉ số khác ở mức chấp nhận được. Các thông số hiệu suất cụ thể sau khi tối ưu:
